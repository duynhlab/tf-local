...
Mỗi case study trong `examples/iam/` nên có đủ 4 sơ đồ:
...
|---|---|---|---|---|---|
| 1 | [SNS→SQS + IRSA](cross-account-sns-sqs-stg/README.md) | `cross-account-sns-sqs-stg/`, `cross-account-sns-sqs/` | floci | Cross-account, cross-region | SNS topic policy, SQS queue policy, IRSA |
| 2 | [EKS Pod → S3](s3-eks/README.md) | `s3-eks/` | floci | Same-account | IRSA vs Pod Identity, ABAC, bucket policy |
| 3 | [Cross-Account AssumeRole](cross-account/README.md) | `cross-account/` | floci | Cross-account | Chained `AssumeRole`, `ExternalId`, confused deputy |
| 4 | [S3 Event Fan-out](s3-events/README.md) | `s3-events/` | floci | Event-driven | 4-layer IAM chain, S3→SNS→SQS |
| 5 | [Cross-Region S3 CRR](cross-region-s3/README.md) | `cross-region-s3/` | floci | Cross-region | Replication IAM, multi-cluster identity, RW/RO |
| 6 | [Cross-Region Pipeline](cross-region-pipeline/README.md) | `cross-region-pipeline/` | floci | Cross-region | SNS delivery, DR queues, multi-region consumers |
| 7 | [ALB Controller on EKS](alb-controller/README.md) | `alb-controller/` | floci runnable | Platform | Controller IAM, RBAC vs IAM, representative ALB lifecycle |
| 8 | [ExternalDNS Cross-Account](external-dns-cross-account/README.md) | `external-dns-cross-account/` | floci runnable | Platform, cross-account | Shared-services DNS, Route53, AssumeRole |
| 9 | [Storage Drivers on EKS](storage-drivers/README.md) | `storage-drivers/` | floci runnable | Platform | EBS CSI resources, EFS IAM shape, pod role isolation |
| 10 | [Secrets Access Cross-Account](cross-account-secrets/README.md) | `cross-account-secrets/` | floci runnable | App workload, cross-account | Role chaining, secret/parameter ARN scoping |
| 11 | [Cluster Access & Break-Glass](cluster-access/README.md) | `cluster-access/` | floci runnable | Human access | IAM guardrails, optional access-entry resources, break-glass admin |
| 12 | [Go BE → S3 Compute Matrix](s3-go-compute-matrix/README.md) | `s3-go-compute-matrix/` | floci runnable | Same/cross-account, same/cross-region | EC2 instance profile, ECS task role, Lambda exec role, AssumeRole + ExternalId |
...
> **Note:** floci giúp validate shape của IAM resources. Nó không chứng minh production enforcement, controller behavior, hay Kubernetes add-on lifecycle như trên AWS thật.
